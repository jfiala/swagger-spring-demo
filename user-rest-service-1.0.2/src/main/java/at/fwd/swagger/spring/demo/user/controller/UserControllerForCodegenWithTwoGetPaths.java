 ...
 */
@RestController
@Api(value="user-codegen-two-get-paths", position=2, description ="User services with two get paths")
public class UserControllerForCodegenWithTwoGetPaths {
	...
	private static final String PATH_USER = "/user_get1_read";
	
	private static final String PATH_SEARCH = "/user_get2_search";
	...
	private static final String MESSAGE_SEARCH_SUCCESS = "Successful retrieval of user list";
    ...
    @RequestMapping(method=RequestMethod.GET, value=PATH_SEARCH)
    @ResponseBody  
    @ApiOperation(value="search for users by name-part", notes="search for users")
    @ApiResponses(value = { 
    	    @ApiResponse(code = 200, message = MESSAGE_SEARCH_SUCCESS, response = User.class) })
